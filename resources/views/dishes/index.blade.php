...
<div id="sidebar-menu" class="hidden lg:grid pl-2 pt-6 text-white font-light text-sm font-main">
    ...
    <a class="py-3 mb-6 pl-2 secondary-color transition-colors duration-300 block rounded-lg hover:bg-[#323035] focus:bg-[#323035] active:bg-[#323035]" href="{{ route('factures.history') }}">Historial de Ventas</a>
        ...
        <div class="grid grid-cols-[70%,20%]">
            <form method="GET" action="{{ route('dishes.index') }}" class="grid grid-cols-3 ">
                <div class="grid">
                    ...
                    <input class="secondary-color rounded text-xs font-light h-8 text-center lg:w-40 sm:w-28 text-white" id="dish" type="text" name="dish" placeholder="Nombre de item">
                </div>
                ...
                <div class="grid">
                    ...
                    <select class="secondary-color rounded h-8 text-center lg:w-40 sm:w-20 text-white" id="category" name="category">
                        <option class="lg:text-base sm:text-xs" value=" value="0" >Todo</option>
            ...
            <div class="content-end">
                <a class="font-bold flex items-center justify-center h-12 lg:w-48 sm:w-32 lg:text-base sm:text-xs  secondary-color text-white rounded-xl text-center hover:bg-gray-500 focus:ring-4 focus:outline-none focus:ring-gray-100" href="{{ route('dishes.create') }}">
                    Agregar un Item
                </a>
            </div>
        </div>
            ...
            <div class="py-10 rounded-lg">
                <table class="w-full rounded-lg">
                    <thead class="rounded-lg text-white font-mainfont-bold secondary-color">
                        <tr>
                            <th scope="col" class="rounded-l-lg px-1 py-3 sm:px-12">Nombre</th>
                            <th scope="col" class="px-1 py-3">Categoria</th>
                            <th scope="col" class="px-5 py-3">Subcategoria</th>
                    ...
                    <tbody>
                        @foreach ($dishes as $dish)
                            <tr class="border-b text-white text-center  border-neutral-200 dark:border-white/10">
                                ...
                                <td class="py-6  ">
                                    <a class="bg-cyan-200 rounded-lg text-black font-semibold px-4 py-2 me-2  sm:mb-4 hover:bg-cyan-500 focus:ring-4 focus:outline-none focus:ring-cyan-100" href="">Ver</a>
                                    ...
                                    <form action="{{ route('dishes.destroy', $dish->id) }}" method="POST" style="display: inline;">
                                        @csrf
                                        @method('DELETE')
                                        <button class="bg-rose-300 rounded-lg text-black font-semibold px-4 py-2 lg:mt-0 sm:mt-4 me-2 hover:bg-rose-500 focus:ring-4 focus:outline-none focus:ring-rose-100" type="submit">Eliminar</button>
                                    </form>
                                </td>
                            </tr>
                        @endforeach
